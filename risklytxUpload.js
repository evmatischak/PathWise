...
const serviceAccount = JSON.parse(
  Buffer.from(process.env.RISKLYTX_SERVICE_ACCOUNT, "base64").toString("utf8")
);
...
async function uploadPathways() {
  ...
  for (const file of files) {
    ...
    console.log(`✅ [Risklytx] Uploaded Pathway: ${pathwayId}`);
  }
}
...
async function uploadOntology() {
  ...
  for (const category of ontology.categories) {
    ...
    console.log(`✅ [Risklytx] Uploaded ${category.terms.length} terms to /ontology/${categoryName}`);
  }
}
